...
function deleteTodo(event) {
    ...
    toDoSave = toDoSave.filter((toDoSave) => toDoSave.id !== parseInt(deletingList.id));
...
if (savedTodos !== null) {
    ...
    //todoSave 변수가 비어 있으면 추가하고 새로고침 할때마다 초기화 되니까 이전 값들을 가지고 있으라고 써준 코드
    parsedTodos.forEach(paintToDo);
    //변수 각각에 함수를 걸어주는 forEach 함수 사용.
}
